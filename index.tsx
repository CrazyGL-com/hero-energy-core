...
function compile(gl: WebGL2RenderingContext, t: number, s: string) {
	...
	if (!gl.getShaderParameter(sh, gl.COMPILE_STATUS)) {
		console.error('[energy-core]', gl.getShaderInfoLog(sh)); throw new Error('compile');
	}
	return sh;
}

function EnergyCoreHero(props: HeroComponentProps) {
	const {
		size, input, reducedMotion,
		coreColor = '#ffd17a', innerColor = '#ffffff', ringColor = '#5cf0ff',
		plasmaColor = '#a26cff', bgColor = '#03020a',
		coreSize = 0.12, coreIntensity = 1.4, ringIntensity = 1,
		plasmaIntensity = 1, bloomStrength = 1, rotationSpeed = 1,
		centerX = 0, transparent = false,
	} = props;
	...
	React.useEffect(() => {
		...
		const gl = c.getContext('webgl2', { antialias: false, alpha: true, premultipliedAlpha: false }); if (!gl) return;
		glRef.current = gl;
		try {
	...
	React.useEffect(() => {
		const c = canvasRef.current, gl = glRef.current; if (!c || !gl) return;
	...
	return (
		<>
			<crazygl-stage style={{ background: transparent ? 'transparent' : bgColor } as React.CSSProperties}>
				<canvas ref={canvasRef} className="crazygl-core-canvas" aria-hidden="true" />
			</crazygl-stage>
...
export default function EnergyCore(props: any) {
	return <CrazyGLWrapper hero={EnergyCoreHero} metadata={metadata as any} {...props} />;
}
export { metadata };
